...
async function getTwitLonger(id){
    ...
    var content = await fetch(`https://twitlonger-api-unofficial.johanstickman.com/twitlonger/${id}`).then(res => res.text())
    return content;
}
...
async function getXlTweet(id){
    ...
    var content = await fetch(`https://twitlonger-api-unofficial.johanstickman.com/xltweet/${id}`).then(res => res.text())
    return content;
}
